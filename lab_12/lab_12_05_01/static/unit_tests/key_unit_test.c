...
START(check_key_works_crctly)
{
    ...
    int function_res = key(arr, 10, arr_lhs, 0);

    ck_assert_int_gt(function_res, ok);
    
    arr_lhs = realloc(arr_lhs, function_res);
    
    ck_assert_ptr_nonnull(arr_lhs);

    int sec_res = key(arr, 10, arr_lhs, &function_res);
    
    ck_assert_int_eq(sec_res, ok);

    for (int *p = arr_lhs, i = 0; p != arr_lhs + function_res; p++)
...
START(check_fun_acpt_invalid_data)
{
    ...
    int function_res = key(arr + 10, 10, arr_lhs, &res_size);
    
    ck_assert_int_ne(function_res, error);
...
START(check_fun_accepts_null_ptr)
{
    ...
    int function_res = key(NULL, 10, arr_lhs, &res_size);
